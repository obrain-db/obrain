...
pub fn run(
    ...
    if dry_run {
        ...
        // T17 final cutover: the legacy tiered-storage epoch-file path
        // (`ObrainDB::compact()`) was retired; substrate handles its own
        // on-disk layout natively, so no epoch files are written here.
