...
impl GraphStore for WalGraphStore {
    ...
    fn get_node_labels(&self, id: NodeId) -> Option<Vec<ArcStr>> {
        // Explicit delegation: without it the trait DEFAULT runs here and
        // calls `self.get_node()` — bulk property hydration to read a label
        // bitset (T12 label pushdown would be silently bypassed).
        self.inner.get_node_labels(id)
    }

    fn node_has_label(&self, id: NodeId, label: &str) -> Option<bool> {
        // Same wrapper rule as get_node_labels above.
