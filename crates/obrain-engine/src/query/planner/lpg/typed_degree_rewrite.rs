...
struct TypedDegreePattern {
    ...
    // Captured for completeness of the extracted pattern; not read yet.
    #[allow(dead_code)]
    group_var: String,
...
fn extract_expand_underneath(op: &LogicalOperator) -> Option<ExpandWithPeerLabel<'_>> {
    ...
    if let LogicalOperator::Expand(expand) = op
        && let LogicalOperator::NodeScan(scan) = expand.input.as_ref()
    {
...
fn extract_typed_degree_pattern(
    ...
    // Limit → Sort
    let LogicalOperator::Sort(sort) = limit.input.as_ref() else {
        return None;
    };
    if sort.keys.len() != 1 {
        return None;
    ...
    let pattern = if agg.aggregates.len() == 1 {
        ...
        let LogicalOperator::LeftJoin(join) = agg.input.as_ref() else {
            return None;
        };
        let LogicalOperator::NodeScan(scan) = join.left.as_ref() else {
            return None;
        };
        ...
        let LogicalOperator::LeftJoin(outer) = agg.input.as_ref() else {
            return None;
        };
        let LogicalOperator::LeftJoin(inner) = outer.left.as_ref() else {
            return None;
        };
        let LogicalOperator::NodeScan(scan) = inner.left.as_ref() else {
            return None;
        };
        if scan.variable != group_var {
            return None;
