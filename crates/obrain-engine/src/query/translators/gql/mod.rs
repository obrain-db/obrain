...
impl GqlTranslator {
    ...
    fn translate_query(&self, query: &ast::QueryStatement) -> Result<LogicalPlan> {
        ...
        // NOTE (T13, gotcha 9ccf0d95): SKIP/LIMIT used to be applied HERE —
        // i.e. BEFORE the ORDER BY wrap below — producing Sort(Limit(…))
        // plans that truncated the input to LIMIT arbitrary rows and then
        // sorted just those. `ORDER BY x ASC LIMIT 1` and `DESC LIMIT 1`
        // returned the same row. They are now applied AFTER wrap_sort, at
        // the end of this function (ORDER BY → SKIP → LIMIT semantics).
        ...
        // Apply SKIP / LIMIT — MUST wrap the Sort: `ORDER BY … SKIP … LIMIT`
        // sorts the full result set first, then paginates (T13 fix).
