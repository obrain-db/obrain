    ...
    #[cfg(feature = "embed")]
    {
        ...
        let run_meta_batch = |batch: &mut Vec<(obrain_common::NodeId, String)>,
         ...
         -> Result<()> {
            ...
            type EmbeddedChunk = Vec<(obrain_common::NodeId, Vec<f32>)>;
            let sub_results: Result<Vec<EmbeddedChunk>> = batch
                .par_chunks(EMBED_BATCH_SIZE)
                .map(|sub| -> Result<EmbeddedChunk> {
...
fn stage_derive_coact(substrate: &Arc<SubstrateStore>, stats: &mut PipelineStats) -> Result<()> {
    ...
    for nid in &node_ids {
        if let Ok(Some(rec)) = writer.read_node(nid.0 as u32)
            && (rec.community_id != 0 || rec.centrality_cached != 0)
        {
            by_community
