...
fn phase_nodes(
    ...
    for (idx, old_id) in all_ids.iter().enumerate() {
        ...
        if nodes_written.is_multiple_of(chunk_every) {
        ...
        let hb_by_count = nodes_written.is_multiple_of(HEARTBEAT_EVERY);
...
fn phase_edges(
    ...
    for src_old in all_ids.iter() {
        ...
        for (dst_old, edge_id) in outgoing {
            ...
            if edges_written.is_multiple_of(chunk_every) {
...
mod tests {
    ...
    // Test helper: the two (edge, props) pairs are clearer as a flat tuple
    // than behind single-use type aliases.
