...
mod tests {
    ...
    #[test]
    #[ignore = "expand_supernode requires create_node_with_id, which SubstrateStore \
                does not implement (slots allocate monotonically; restoring a \
                specific id would need slot un-tombstoning). Broken since LpgStore \
                removal (T17 W4) — un-ignore once substrate supports slot restore."]
