...
impl SubstrateStore {
    ...
    fn load_properties(&self) -> SubstrateResult<()> {
        ...
        let skip = |k: &str| SKIP_ON_LOAD_PROP_KEYS.contains(&k);
            ...
            let partials: Vec<FxHashMap<NodeId, EdgeId>> = (1..edge_hw)
                ...
                .fold(FxHashMap::<NodeId, EdgeId>::default, |mut acc, slot| {
                    if let Ok(Some(rec)) = self.writer.read_edge(slot)
                        && rec.flags & edge_flags::TOMBSTONED == 0
                    {
            ...
            let partials: Vec<FxHashMap<u32, CidRange>> = (1..node_hw)
                ...
                .fold(FxHashMap::<u32, CidRange>::default, |mut acc, slot| {
                    if let Ok(Some(rec)) = self.writer.read_node(slot)
                        && rec.flags & crate::record::node_flags::TOMBSTONED == 0
                    {
    ...
    fn allocate_node_id_in_community(&self, community_id: u32) -> NodeId {
        ...
        loop {
            ...
            let aligned = if hw <= 1 || hw.is_multiple_of(NODES_PER_PAGE) {
                hw.max(1)
            } else {
                // ceil(hw / NODES_PER_PAGE) * NODES_PER_PAGE
                hw.div_ceil(NODES_PER_PAGE) * NODES_PER_PAGE
            };
            if self
                .next_node_id
...
impl SubstrateStore {
    ...
    fn read_all_properties_for_node(
        ...
        if let Some(pz_lock) = self.props_zone.as_ref()
            && let Some(head) = decode_page_id(rec.first_prop_off)
        {
            let pz = pz_lock.read();
            let mut live: std::collections::HashMap<u16, crate::page::PropertyEntry> =
                std::collections::HashMap::new();
            let mut tombstoned: std::collections::HashSet<u16> = std::collections::HashSet::new();
            if let Err(err) = pz.collect_live_props_by_key(Some(head), &mut live, &mut tombstoned) {
                tracing::warn!(
                    node_id = id.0,
                    error = %err,
...
impl GraphStore for SubstrateStore {
    ...
    fn get_node_labels(&self, id: NodeId) -> Option<Vec<ArcStr>> {
        // T12 label pushdown — O(1) label read from the mmap'd NodeRecord
        // bitset + registry, WITHOUT the 4-zone property hydration the
        // trait default (`get_node`) would pay. `hasLabel()` / `labels()`
        // in the Cypher evaluator route through this: a 4-label WHERE on
        // 85-prop nodes went from full bulk-hydrate per row to a bit test
        // (list_project_symbols 21.7 s → see bench note 96ae814c).
        if !self.is_live_on_disk(id) {
            return None;
        }
        let rec = self.writer.read_node(id.0 as u32).ok().flatten()?;
        Some(
            self.resolve_node_labels_from_bitset(rec.label_bitset)
                .into_iter()
                .collect(),
        )
    }

    fn node_has_label(&self, id: NodeId, label: &str) -> Option<bool> {
        // T13/T12 hot path — pure bit test: no property hydration, no
        // label-list allocation, no string compares. Measured impact: the
        // WHERE `s:Function OR s:Struct OR s:Trait OR s:Enum` filter over
        // 199k candidate rows evaluates up to 4 hasLabel per row.
        if !self.is_live_on_disk(id) {
            return None;
        }
        let rec = self.writer.read_node(id.0 as u32).ok().flatten()?;
        let Some(bit) = self.labels.read().bit_for(label) else {
            // Unknown label on this base: the node exists, the label
            // cannot be present.
            return Some(false);
        };
        Some(rec.label_bitset & (1u64 << bit) != 0)
    }
    ...
    fn get_node_property(&self, id: NodeId, key: &PropertyKey) -> Option<Value> {
        ...
        if let Some(v) = self.lookup_node_property_v2(id, key.as_str()) {
            return v;
        }
    ...
    fn get_edge_property(&self, id: EdgeId, key: &PropertyKey) -> Option<Value> {
        ...
        //   - Some(None)    → tombstone wins (deleted; skip DashMap)
        //   - None          → nothing on v2 chain; fall through
        if let Some(v) = self.lookup_edge_property_v2(id, key.as_str()) {
            return v;
        }
    ...
    fn nodes_by_label(&self, label: &str) -> Vec<NodeId> {
        // usize::MAX = "no cap" — full scan as before.
        self.nodes_by_label_capped(label, usize::MAX)
    }

    /// LIMIT-aware label scan. Stops as soon as `max` nodes are collected,
    /// saving O(N) work for queries like `MATCH (n:ChatEvent) RETURN n LIMIT 5`
    /// where N is the live count of a big label (e.g. 681K ChatEvents on po).
    /// Before this : `LIMIT 5` still scanned all 1.35M slots → 3-4s.
    /// After this  : the scan breaks after 5 matches → microseconds.
    fn nodes_by_label_capped(&self, label: &str, max: usize) -> Vec<NodeId> {
        if max == 0 {
            return Vec::new();
        }
        ...
        let hw = self.next_node_id.load(Ordering::Acquire);
        let mut out: Vec<NodeId> = Vec::new();
        // Cap the Vec allocation when max is small (avoids over-reserve for
        // small LIMITs while letting the default uncapped path grow as needed).
        if max < 4096 {
            out.reserve(max);
        }
        for slot in 1..hw {
...
impl GraphStoreMut for SubstrateStore {
    ...
    fn set_node_property(&self, id: NodeId, key: &str, value: Value) {
        ...
        if self.props_zone.is_some()
            && let Err(err) = self.append_scalar_to_props_zone_v2(id, key, &value)
        {
            tracing::warn!(
                node_id = id.0,
                key = %key,
    ...
    fn set_edge_property(&self, id: EdgeId, key: &str, value: Value) {
        ...
        if self.props_zone.is_some()
            && let Err(err) = self.append_scalar_to_props_zone_v2_edge(id, key, &value)
        {
            tracing::warn!(
                edge_id = id.0,
                key = %key,
    ...
    fn remove_node_property(&self, id: NodeId, key: &str) -> Option<Value> {
        ...
        if self.props_zone.is_some()
            && let Err(err) = self.append_tombstone_to_props_zone_v2(id, key)
        {
            tracing::warn!(
                node_id = id.0,
                key = %key,
    ...
    fn remove_edge_property(&self, id: EdgeId, key: &str) -> Option<Value> {
        ...
        // semantics but must not lose the delete.
        if self.props_zone.is_some()
            && let Err(err) = self.append_tombstone_to_props_zone_v2_edge(id, key)
        {
            tracing::warn!(
                edge_id = id.0,
                key = %key,
...
mod tests {
    ...
    fn auto_migrates_legacy_blobs_on_load() {
        ...
        assert!(big_string.len() > BLOB_COLUMN_THRESHOLD_BYTES);
    ...
    fn coact_reinforce_f32_saturates_and_reads_back() {
        ...
        let e2 = EdgeRecord {
            src: 3,
            dst: 4,
            edge_type: coact_id.wrapping_add(1), // anything but COACT
            weight_u16: 0xFFFF,
            ..Default::default()
        };
    ...
    fn decay_all_coact_only_touches_coact_typed_edges() {
        ...
        for slot in 1..=6u64 {
            let e = EdgeRecord {
                src: slot as u32,
                dst: (slot as u32) + 100,
                edge_type: if slot <= 4 { coact_id } else { other_id },
                weight_u16: 0x8000,
                ..Default::default()
            };
    ...
    /// then `flush` + sidecar delete, a fresh reopen resolves every
    ...
    fn finalize_props_v2_drains_legacy_sidecar_end_to_end() {
        ...
        unsafe { std::env::set_var("OBRAIN_PROPS_V2", "1") };
        {
            ...
            assert!(!s.node_properties.is_empty());
    ...
    fn finalize_props_v2_preserves_edge_properties() {
        ...
        unsafe { std::env::set_var("OBRAIN_PROPS_V2", "1") };
        {
            ...
            assert!(s.props_v2_enabled());
            assert!(!s.node_properties.is_empty());
            assert!(!s.edge_properties.is_empty());
