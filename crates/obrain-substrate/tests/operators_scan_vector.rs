...
// TODO(revisit): obrain-substrate declares no `vector-index` feature, so this
// cfg never evaluates true here (the feature lives in obrain-core). The test
// stays dormant until the feature is plumbed through this crate's Cargo.toml.
// The `allow` lives on a wrapper module because an allow on the cfg-stripped
// item itself would be stripped along with it.
